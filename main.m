...
% Controle de qualidade em frutos (limões) 
...
%
%% Código da aplicação por operação morfológicas
% Leitura da imagem
imagem = imread('dataset/good_quality_258.jpg');
% imagem = imread('dataset/0016_A_H_15_C.jpg');
...
%% Código da aplicação por rede neural (já treinada)
% % Carregar o modelo treinado
% load('classificacao_limao.mat', 'net');
% 
% % Leitura da imagem
% imagem = imread('dataset/0003_B_I_0_G.jpg');
...
% figure; imshow(imagem);
% 
% % Fazer a predição na imagem específica
% resultadoImagemSelecionada = classify(net, imagem);
% resultado = char(resultadoImagemSelecionada);
% 
% if strcmpi(resultado, 'good_quality')
%     disp('Limão com boa qualidade.');
% elseif strcmpi(resultado, 'bad_quality')
%     disp('Limão com mofo.');
% elseif strcmpi(resultado, 'empty_background')
%     disp('Imagem sem limão.');
% end
